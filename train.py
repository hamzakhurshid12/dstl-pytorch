...
class Model:
    ...
    def train(self, logdir: Path, train_ids: List[str], valid_ids: List[str],
        ...
        #train_ids=train_ids[2:] #for limited RAM JG
        ...
        for n_epoch in range(start_epoch, self.hps.n_epochs):
            ...
            gloabal_step=self.net.global_step[0]
    ...
    def _train_on_feeds(self, gen_batch, n_batches: int, no_mp: bool):
        ...
        for i, (x, y, dist_y) in enumerate(map_(gen_batch, range(n_batches))):
            if losses[0] and i % log_step == 0:
                ...
                pred_y = self.net(self._var(x)).data.cpu() #line 77
                #batch_correct = (y.eq(pred_y.long())).double().sum().item() #JG
                print('Batch correct value is: {}\n'.format(batch_correct))
    ...
    def _log_value(self, name, value):
        self.tb_logger.log_value(name, value, step=gloabal_step)
    ...
    def predict_image_mask(self, im_data: np.ndarray,
        ...
        step = s // 3 #if average_shifts else sshould be an integer, got <class 'torch.Tensor'>
